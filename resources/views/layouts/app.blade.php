    ...
    <header class="sticky top-0 z-40 border-b border-stone-200 bg-white/95 backdrop-blur">
        <div class="mx-auto flex max-w-7xl items-center gap-2 px-3 py-2.5 sm:gap-4 sm:px-4 sm:py-3">
            <a href="{{ route('home') }}" class="flex shrink-0 items-center gap-2">
                <img src="{{ asset('images/logo.png') }}" alt="{{ $storeSetting->store_name ?? 'Allo! Pizza' }}" class="h-10 w-10 object-contain sm:hidden">
                <img src="{{ asset('images/logo-wide.png') }}" alt="{{ $storeSetting->store_name ?? 'Allo! Pizza' }}" class="hidden h-10 w-auto max-w-[180px] object-contain sm:block md:max-w-[220px] md:h-11">
            </a>
            ...
            <nav class="ml-auto flex items-center gap-1 text-xs font-medium sm:gap-3 sm:text-sm">
                <a href="{{ route('menu') }}" class="rounded-lg px-2 py-2 text-stone-700 hover:bg-stone-100 sm:px-3">Меню</a>
                <a href="{{ route('lunch.index') }}" class="hidden rounded-lg px-2 py-2 text-stone-700 hover:bg-stone-100 sm:inline-flex sm:px-3">Обедно меню</a>
                ...
                <a href="{{ route('cart') }}"
                   class="relative inline-flex items-center gap-2 rounded-xl bg-brand-500 px-3 py-2 font-semibold text-white shadow-soft transition hover:bg-brand-600 sm:px-4">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17M17 13v4a2 2 0 11-4 0v-4m4 0H9"/></svg>
